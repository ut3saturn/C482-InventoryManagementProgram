...
public class AddProductController implements Initializable {
    ...
    public void searchPartBtnPressed() throws Exception {
        ...
        if (!partSearchTF.getText().trim().isEmpty()){
            for(Part p : inv.getAllParts()){
                if(p.getPartName().toLowerCase().contains(partSearchTF.getText().toLowerCase().trim())){
                    searchedParts.add(p);
                }
            }
    ...
    // Disable the add and remove buttons until a row is selected
    ...
    private void errorWindow(int errorCode) {
